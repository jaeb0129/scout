...
export default function PitcherTable({ players, onSelectPlayer, metricsConfig }) {
  if (players.length === 0) {
  ...
  return (
    <div className="table-wrap">
      <div className="table-head pitcher-grid" style={{ "--stat-cols": allCols.length }}>
        <div>PITCHER</div>
        <div>BIO</div>
        {allCols.map((c) => (
...
function Row({ p, onSelectPlayer, allCols }) {
  ...
  return (
    <div className={`row pitcher-grid ${statusClass(p.derivedStatus)}`} style={{ "--stat-cols": allCols.length }}>
      <div
