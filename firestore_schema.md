...
meta/playerIds
{
  ids: number[],      // players 컬렉션에 등록된 모든 선수 id (sync.py가 로스터 동기화 끝날 때마다 갱신)
  updatedAt: timestamp,
}
```
`scripts/sync_metrics.py`가 "이 id가 지금 추적 대상인지" 확인할 때 players 컬렉션 전체를
스캔하지 않고 이 문서 1개만 읽도록 하기 위한 인덱스. 컬렉션 전체 스캔은 선수 수만큼 읽기로
